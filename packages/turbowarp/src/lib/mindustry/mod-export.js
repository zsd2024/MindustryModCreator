...
function getContentFolder(contentType) {
  return contentTypeToFolder[contentType] || 'blocks';
}

function findFolderPath(folders, folderId) {
  const parts = [];
...
function buildContentHjson(data) {
  const clean = {};
...
export default function exportMod(assets, folders, modConfig, assetFormData) {
  const zip = new JSZip();
  ...
  for (const asset of javaAssets) {
    ...
    zip.file(`${dir}${asset.name}.java`, `// ${asset.name}.java\n// Java source code\npackage ${pkg.replace(/\//g, '.') || 'your'};\n\n`);
  }
