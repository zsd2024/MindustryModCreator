...
export class DockerSandboxService {
  ...
  async compile(buildDir: string): Promise<any> {
    ...
    try {
      ...
      this.logger.error(`编译失败: ${containerName}`, error instanceof Error ? error.stack : '');
      await this.cleanup(containerName);
      throw error;
    }
  }
  ...
  private async cleanup(containerName: string): Promise<void> {
    try {
      ...
      // 删除卷
      await this.docker
        .getVolume(`${containerName}-output`)
        .remove()
        .catch(() => {});
      await this.docker
        .getVolume(`${containerName}-classes`)
        .remove()
        .catch(() => {});

      this.logger.log(`清理完成: ${containerName}`);
    } catch (error) {
      this.logger.error(`清理失败: ${containerName}`, error instanceof Error ? error.stack : '');
    }
  }
}
