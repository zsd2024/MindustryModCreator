...
export class DirectoryBuilderService {
  ...
  async buildDirectory(manifest: any): Promise<string> {
    ...
    await fs.mkdir(path.join(buildDir, 'src', 'com', 'mymod', 'content', 'blocks'), {
      recursive: true,
    });

    // 生成 mod.json
    const modJson = this.generateModJson(manifest.modMeta);
    await fs.writeFile(path.join(buildDir, 'mod.json'), JSON.stringify(modJson, null, 2));
  ...
  async cleanup(buildDir: string): Promise<void> {
    try {
      ...
      this.logger.error(`清理构建目录失败: ${buildDir}`, error instanceof Error ? error.stack : '');
    }
  }
}
